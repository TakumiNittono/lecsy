...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use the user's supabase client (admin RLS policy allows reading all reports)
    ...
    if (fetchError) {
      ...
      return NextResponse.json({ error: 'Failed to fetch reports', details: fetchError.message }, { status: 500 })
    }

    return NextResponse.json({ reports: data })
  } catch (err) {
    console.error('Reports API error:', err instanceof Error ? err.message : err)
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Use user's supabase client (admin RLS policy allows updating all reports)
